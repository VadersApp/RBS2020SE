...
extern "C" {
	...
	void SKSEMessageHandler(SKSEMessagingInterface::Message* message) {
		...
		case SKSEMessagingInterface::kMessage_PostLoad: {
			...
			g_messaging->Dispatch(
				g_pluginHandle, InterfaceExchangeMessage::kMessage_ExchangeInterface,
				(void*)&message, sizeof(InterfaceExchangeMessage*), "skee");
			if (message.interfaceMap) {
				g_bodyMorphInterface =
					(IBodyMorphInterface*)message.interfaceMap->QueryInterface(
						"BodyMorph");
	...
	bool SKSEPlugin_Query(const SKSEInterface* a_skse, PluginInfo* a_info) {
		...
		gLog.OpenRelative(CSIDL_MYDOCUMENTS,
			"\\My Games\\Skyrim Special Edition\\SKSE\\RBS2020.log");
	...
	bool SKSEPlugin_Load(const SKSEInterface* a_skse) {
		...
		g_messaging =
			(SKSEMessagingInterface*)a_skse->QueryInterface(kInterface_Messaging);
		if (g_messaging->RegisterListener(g_pluginHandle, "SKSE",
			SKSEMessageHandler)) {
		...
		g_papyrus =
			(SKSEPapyrusInterface*)a_skse->QueryInterface(kInterface_Papyrus);
